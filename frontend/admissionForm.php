...
#***********************************This will check if any sibling of new admission already in School************
           $query="SELECT * from Parent where (FATERCNIC='$fatherCNIC' OR MOTHERCNIC='$motherCNIC')";
            $parsedQuery=oci_parse($con,$query);
             $exceutionResult=oci_execute($parsedQuery);
              $numofRows=oci_fetch_all($parsedQuery,$numofRows);
              if($numofRows>0)
              {
                $query="SELECT * from Parent where (FATHERNAME=' $fatherName' AND FATHERADDRESS='$fatherAddress'AND FATHERPHONENO='$fatherContact' AND MOTHERNAME='$$motherName')";
                 $parsedQuery=oci_parse($con,$query);
                  $exceutionResult=oci_execute($parsedQuery);
                   $numofRows2=oci_fetch_all($parsedQuery,$numofRows2);
                    ...
                    if($numofRows==0)
                    {
                      echo "Either your child is a student of Hamaray Bachay(other info is incorrect), else your CNICs are incorrect!";
                    }
                    else if($numofRows>0)
                    {
                      #this queray gets the parent primary key beacause this admission is a sibiling of a student
                       $guad="SELECT parentid from parent where FATERCNIC='$fatherCNIC' OR MOTHERCNIC='$motherCNIC'";
                        $guadParse=oci_parse($con,$guad);
                         $exceutionResult=oci_execute($guadParse);
                          $row = oci_fetch_array($guadParse); 
                           $parentIDExisting = $row[0];
...
?>
>>>>>>> 67251e353e5c27400acb40ace20b3d2567d53bbb
</div>